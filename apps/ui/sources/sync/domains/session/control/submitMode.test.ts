...
describe('selectSessionPendingRequestedAction', () => {
    ...
    it('keeps inactive and stale-active offline input FIFO unless timing is explicitly overridden', () => {
        ...
        expect(selectSessionPendingRequestedAction({ session: staleActiveOffline, nowMs: now })).toEqual({
            v: 1,
            kind: 'enqueue',
        });
        expect(selectSessionPendingRequestedAction({
