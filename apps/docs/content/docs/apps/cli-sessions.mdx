...
For bounded maintenance scans on development builds, use:

```bash
happier session list --resumable-health --json
```

This compact path returns stopped, unarchived session ids, active state, whether a materialized pending permission or user-action request needs the user, meaningful activity time, and a cursor. It does not decrypt or return rich session rows. `needsUserAction` is narrower than the session list's general **Needs attention** state: unread messages and failed turns do not set it.
...
Pin / unpin, and read the pinned set back:

```bash
happier session pin <session-id-or-prefix>
happier session unpin <session-id-or-prefix>
happier session pins
```

Both writes are idempotent: pinning an already-pinned session and unpinning an
unpinned one both succeed and change nothing. `happier session pins` reports
what the server holds, so an automation can verify a pin rather than trust the
write response. The server only reports a pin for a visible, unarchived
session.
