...
Only one daemon/controller can own a server profile at a time. A second controller reports the exact conflicting process instead of starting another worker owner. Different server profiles remain independent.
...
On development builds, daemon status also reports local health alerts for worker count, daemon and worker RSS, swap use, queued session-list requests, startup worker reconciliation, and open quota-persistence circuits. These alerts stay local; Happier does not send an external notification from this health path.
