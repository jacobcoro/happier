...
Development builds also provide `happier session list --resumable-health --json` for bounded maintenance scans. It returns only stopped, unarchived session ids, active state, the server-readable `needsUserAction` state, meaningful activity time, and page cursors. It deliberately does not load encrypted metadata, Agent state, resume eligibility, shares, or transcript previews.
...
happier session pin <session-id>
happier session unpin <session-id>
