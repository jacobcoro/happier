...
Development builds can create a local Git worktree before launch:

```bash
happier session create --path /path/to/repo --worktree feature/auth --worktree-base upstream/dev
```

The JSON result reports the worktree, session path, branch, repository roots, and cleanup disposition.
See [CLI sessions](/apps/cli-sessions#create-in-a-managed-git-worktree) for cleanup and retry rules.
