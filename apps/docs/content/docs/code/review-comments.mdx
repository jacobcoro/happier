...
<Callout type="info">
  In the current development build (unreleased), Review shows how many comment
  drafts are saved and how many are included in the next prompt. Choose **Go to
  composer** to return to the conversation with your review tabs and drafts kept.
  This opens the composer; it does not send the comments. Use the **Review** chip
  to inspect the selection before sending.
</Callout>
